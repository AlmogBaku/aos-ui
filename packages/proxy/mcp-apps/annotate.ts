...
async function* annotatedEvents(
  ...
  for await (const event of events) {
    if (event.type === RunEventKind.TOOL_CALL_START) {
      const { toolCallId, toolCallName: toolName } = event
    ...
    if (
      event.type === RunEventKind.TOOL_CALL_END &&
      args.has(event.toolCallId)
    ) {
    ...
    if (event.type !== RunEventKind.TOOL_CALL_RESULT) {
      yield event
      continue
    }
    ...
    const app =
      ...
      (toolName !== undefined &&
        ...
        (await describeWithin(apps, scope, {
          toolCallId: event.toolCallId,
          toolName,
          result: parseJson(event.content),
        })))
    if (app && toolName !== undefined)
      apps.observe?.(scope, {
        toolCallId: event.toolCallId,
        toolName,
        result: liveResult(event.content),
      })
...
function annotatedRuns(
  runs: ServerRunEngine,
  apps: ServerMcpApps
): ServerRunEngine {
  return {
    start: async (scope, input, attachments) =>
      annotatedHandle(await runs.start(scope, input, attachments), apps, scope),
    recover: async (scope, request) =>
      annotatedHandle(await runs.recover(scope, request), apps, scope),
    ...(runs.discover
      ? {
          discover: async (scope: SessionScope, runId: string) => {
            const found = await runs.discover!(scope, runId)
            return (
              found && {
                ...found,
...
export function withMcpApps<Runtime extends ServerRuntime>(
  ...
  const runs = annotatedRuns(native.runs, apps)

  const overrides: Pick<
    ServerRuntime,
    "runs" | "history" | "workspaceCapabilities"
  > = {
    runs,
