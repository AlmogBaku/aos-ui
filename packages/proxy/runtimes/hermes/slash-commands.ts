...
export async function nativeSlashCommands(
  ...
  try {
    ...
    if (!value || typeof value !== "object" || !("pairs" in value) || !Array.isArray(value.pairs)) return []
    const commands: SlashCommand[] = []
    const seen = new Set<string>()
    for (const pair of value.pairs.slice(0, 10_000)) {
      if (!Array.isArray(pair) || pair.length !== 2 || typeof pair[0] !== "string") continue
      const parsed = SlashCommandSchema.safeParse({ name: pair[0].replace(/^\//u, ""), description: pair[1] })
...
export function slashInvocation(text: string, commands: readonly SlashCommand[]) {
  const match = /^\/([^\s/]+)(?:\s+([\s\S]*))?$/u.exec(text)
  if (!match || !commands.some(({ name }) => name === match[1])) return undefined
