...
it("projects native slash catalog names in order without duplicate or malformed entries", async () => {
  ...
  await expect(
    adapter.slashCommands("writer", "hermes:writer:stored")
  ).resolves.toEqual({
    commands: [
...
it("routes an exact native command to slash.exec and exposes synchronous text", async () => {
  ...
  await expect(
    adapter.submit("live", { text: "/help details", runId: "run" })
  ).resolves.toEqual({
...
it.each([-32601, 4018])(
  ...
  async (code) => {
    ...
    const adapter = new HermesServerAdapter({ request })
    await expect(
      adapter.submit("live", { text: "/skill arguments", runId: "run" })
...
it("keeps command-looking attachment turns on chat but rejects failed discovery", async () => {
  ...
  await expect(
    adapter.submit("live", { text: "/help", runId: "two" })
