...
describe("OpenClaw run engine AOS tools", () => {
  ...
  it("fails the turn without sending when enabling the MCP server fails", async () => {
    const native = new ControlledNative()
  ...
  it("names prefixed AOS tools canonically and keeps unknown MCP tools raw", async () => {
    ...
    expect(
      events
        .filter(({ kind }) => kind === TurnEventKind.ToolCallStarted)
        .map(({ name }) => name)
  ...
  it("publishes a present_artifact receipt without its native path", async () => {
    ...
    const artifact = events.find(
      ({ kind }) => kind === TurnEventKind.ArtifactPublished
    )
    expect(artifact?.artifact).toEqual({
      id: expect.stringMatching(/^openclaw-artifact-[a-f0-9]{32}$/u),
      filename: "report.pdf",
      mimeType: "application/pdf",
      source: {
        type: "provider",
        reference: (artifact?.artifact as { id: string }).id,
      },
    })
    const result = events.find(
      ({ kind }) => kind === TurnEventKind.ToolCallFinished
    )
    expect(JSON.parse(result?.output as string)).toEqual({
      ok: true,
      type: "aos.artifact",
      artifact: {
        id: (artifact?.artifact as { id: string }).id,
        filename: "report.pdf",
        mimeType: "application/pdf",
      },
    })
  ...
  it("publishes no artifact for a receipt with an unsafe path", async () => {
    ...
    expect(
      events.some(({ kind }) => kind === TurnEventKind.ArtifactPublished)
    ).toBe(false)
  })
})
