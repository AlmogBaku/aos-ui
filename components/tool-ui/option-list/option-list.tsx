  ...
  return (
    <div
      className={cn(
        ...
        "text-foreground",
        "motion-safe:animate-in motion-safe:duration-300 motion-safe:ease-[cubic-bezier(0.16,1,0.3,1)] motion-safe:fill-mode-both motion-safe:zoom-in-95 motion-safe:blur-in-sm motion-safe:fade-in",
        className
      )}
...
export function OptionList({
  ...
  useEffect(() => {
    ...
    // Keep the roving tab stop valid when provider-updated options remove or
    // disable the active item. This state intentionally follows external
    // option availability rather than deriving a new selection.
  ...
  const normalizedFooterActions = useMemo(() => {
    // An explicitly empty array is a valid runtime choice for selection-only
    // flows. Keep it distinct from an omitted configuration, which gets the
    // registry component's default Clear/Confirm controls.
    if (Array.isArray(actions) && actions.length === 0) {
      return { items: [], align: "right" as const }
    }
