...
describe("WorkspaceShell", () => {
  ...
  it("renders provisional Agents with an unassigned icon and optional recovery actions", async () => {
    const user = userEvent.setup()
    const onRetryAgentDraft = vi.fn()
    const onDeleteAgentDraft = vi.fn()
    const draftAgent: WorkspaceAgent = {
      id: "draft-agent",
      kind: "provisional",
      builderThreadId: "thread-builder",
      phase: "start-failed",
      name: "New Agent",
      description: "Finishing your Agent setup",
      lastError: "The provider disconnected",
      status: "attention",
      icon: { kind: "symbol", symbol: "unassigned", tone: "teal" },
    }

    const { container } = renderShell({
      agents: [draftAgent],
      selectedAgentId: draftAgent.id,
      onRetryAgentDraft,
      onDeleteAgentDraft,
    })

    expect(
      container.querySelector('[data-agent-symbol="unassigned"]')
    ).toBeInTheDocument()
    const inspector = screen.getByRole("complementary", {
      name: "Agent details",
    })
    expect(
      within(inspector).getByText("Couldn’t start the interview")
    ).toBeVisible()
    expect(
      within(inspector).getByText("The provider disconnected")
    ).toBeVisible()

    await user.click(
      within(inspector).getByRole("button", { name: "Retry Agent creation" })
    )
    ...
    expect(onDeleteAgentDraft).not.toHaveBeenCalled()
    expect(deleteTrigger).toHaveFocus()

    await user.click(deleteTrigger)
    await user.click(screen.getByRole("button", { name: "Delete draft" }))

    expect(onRetryAgentDraft).toHaveBeenCalledWith("draft-agent")
    expect(onDeleteAgentDraft).toHaveBeenCalledWith("draft-agent")
  })
