...
func projectDisplay(id, tool string, input json.RawMessage) *conversation.Display {
	kind := displayKinds[tool]
	if kind == "" || id == "" || len(id) > 256 || len(input) == 0 || len(input) > maxDisplayPayloadBytes {
		return nil
	}
	valid := false
	switch kind {
...
func successfulPresentationResult(row map[string]any) bool {
	if failed, _ := row["is_error"].(bool); failed {
		return false
	}
	value, ok := row["content"]
	if !ok {
		value, ok = row["result"]
	}
	if !ok {
	...
	if len(raw) == 0 || len(raw) > maxDisplayPayloadBytes {
		return false
	}
	var result struct {
		OK bool `json:"ok"`
