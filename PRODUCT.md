...
- On configured real runtimes, Agent creation opens a creator-owned Session that the browser alone projects as a `New Agent` draft row in the Agent rail with exactly one Session, until the creator tool reports its outcome. The draft persists until the Session is deleted (Discard draft) or 48 hours since last activity; selecting a draft shows no Session tab strip and no inspector. The hidden creator identity never appears in the ordinary roster or management catalog. Creation uses portable guidance with a native safe writer; the created Agent's profile starts hidden and is revealed only after successful setup — if plugin setup fails the result is `setup-needed` and an operator must finish setup to make the Agent visible in Manage Agents. There is no ownership transfer and no automatic first Session for the created Agent. Public fixture/demo mode intentionally omits Agent creation. Native harnesses can initiate inbound Sessions without a browser.
...
- An Artifact is a provider-owned deliverable that an Agent explicitly publishes with `present_artifact` or a trusted provider-native delivery tool such as Hermes text-to-speech; ordinary file edits and assistant-authored paths are not Artifacts. AOS derives published outputs from the active conversation branch and provides read-only preview and download without owning file editing, storage, or version history. Published image, audio, and video Artifacts render inline in conversation.
