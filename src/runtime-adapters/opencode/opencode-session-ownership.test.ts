...
describe("createAgentScopedOpenCodeClient", () => {
  it("reconciles authoritative root ownership after reconnect", async () => {
    const { client, get, promptAsync } = createClient()
    const ownership = new OpenCodeSessionOwnership()
    ownership.remember("stale-session", "old")
    ownership.reconcile([
      ["session-build", "build"],
    ...
    await expect(ownership.find(client, "stale-session")).resolves.toBe("build")
