...
function toRemoteThread(session: AgUiSessionMetadata) {
  return {
    ...
    custom: { agentId: session.agentId },
  }
}

async function unsupportedMutation() {
  throw new Error("AG-UI Session mutation is not supported")
}
...
export class AgUiThreadListBridge {
  ...
  readonly #threadAgents = new Map<string, AbstractAgent>()
  readonly #threadHistories = new Map<string, ThreadHistoryAdapter>()
  ...
  /** One public AG-UI agent clone per native Session. */
  agentForThread(threadId: string) {
    let agent = this.#threadAgents.get(threadId)
    if (!agent) {
      agent = this.#agent.clone() as AbstractAgent
      if (!agent || agent === this.#agent) {
        throw new Error(
          "AG-UI multi-session runtime requires agent.clone() to return an independent agent"
        )
      }
      this.#threadAgents.set(threadId, agent)
    }
    agent.threadId = threadId
    agent.agentId = this.#snapshot.sessions.find(
      (session) => session.threadId === threadId
    )?.agentId
    return agent
  }

  historyForThread(threadId: string): ThreadHistoryAdapter {
    let history = this.#threadHistories.get(threadId)
    if (history) return history
    history = {
      load: async () => {
        const snapshot = await this.#transport.loadSession(threadId)
        return toRepository({
          ...snapshot,
          unstableResume: Boolean(
            snapshot.unstableResume && this.#transport.resumeSession
          ),
        })
      },
      append: async () => {
        // AG-UI sends canonical history with each run; persistence is native.
      },
      ...(this.#transport.resumeSession
        ? {
            resume: (options) =>
              this.#transport.resumeSession!(threadId, options),
          }
        : {}),
    }
  ...
  readonly select = (threadId: string | undefined) => {
    if (!threadId) {
      this.#agent.threadId = ""
      this.#agent.agentId = undefined
      this.#setSnapshot({ ...this.#snapshot, activeThreadId: undefined })
      return
    }
    ...
    if (!session) return
    this.#retargetAgent(threadId, this.#snapshot.sessions)
    this.#setSnapshot({ ...this.#snapshot, activeThreadId: threadId })
  }

  readonly threadListAdapter: RemoteThreadListAdapter = {
    list: async () => {
      await this.initialize()
      return { threads: this.#snapshot.sessions.map(toRemoteThread) }
    },
    fetch: async (threadId) => {
      await this.initialize()
      let session = this.#snapshot.sessions.find(
        (item) => item.threadId === threadId
      )
      if (!session) {
        await this.refresh()
        session = this.#snapshot.sessions.find(
          (item) => item.threadId === threadId
        )
      }
      if (!session) throw new Error(`AG-UI Session not found: ${threadId}`)
      return toRemoteThread(session)
    },
    initialize: async () => {
      throw new Error("Create AG-UI Sessions through the workspace")
    },
    rename: unsupportedMutation,
    archive: unsupportedMutation,
    unarchive: unsupportedMutation,
    delete: unsupportedMutation,
    generateTitle: async () => {
      throw new Error("AG-UI Session title generation is not supported")
    },
  }
