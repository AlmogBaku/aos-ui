      ...
      {resolvedActions.map((action) => {
        ...
        return (
          ...
          >
            {action.isLoading && (
              <svg
                className="me-2 h-4 w-4 motion-safe:animate-spin"
            ...
            {action.shortcut && !action.isLoading && (
              <kbd className="ms-2.5 hidden rounded-lg border border-border bg-muted px-2 py-0.5 font-mono text-xs font-medium sm:inline-block">
                {action.shortcut}
              </kbd>
            )}
          </Button>
