...
export type PlanTodo = z.infer<typeof PlanTodoSchema> & {
  /** Runtime-only status copy; excluded from the serializable tool contract. */
  statusLabel?: string
}
...
export type PlanProps = z.infer<typeof PlanPropsSchema> & {
  className?: string
  /** Runtime-only scope caption; excluded from the serializable tool contract. */
  caption?: string
  /** Runtime presentation only; excluded from the serializable tool contract. */
  headingLevel?: 2 | 3
  /** Runtime-only copy; excluded from the serializable tool contract. */
