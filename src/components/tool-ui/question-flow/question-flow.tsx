...
function ProgressBar({ current, total }: ProgressBarProps) {
  ...
  return (
    <div
      ...
      role="progressbar"
      aria-label={labels.questionProgressLabel}
    ...
    >
      {Array.from({ length: total }).map((_, i) => (
        ...
        >
          <div
            className={cn(
              "origin-start absolute inset-0 rounded-full bg-primary",
  ...
  return (
    <div
      className={cn(
        ...
        isSelected && [
          ...
          "motion-safe:animate-in motion-safe:duration-300 motion-safe:ease-out motion-safe:zoom-in-75 motion-safe:fade-in",
        ],
    ...
    >
      {mode === "multi" && isSelected && (
        <Check
          className="size-3 motion-safe:animate-in motion-safe:delay-75 motion-safe:duration-200 motion-safe:fill-mode-both motion-safe:zoom-in-75 motion-safe:fade-in"
          strokeWidth={3}
        />
      )}
      {mode === "single" && isSelected && (
        <span className="size-2 rounded-full bg-current motion-safe:animate-in motion-safe:duration-300 motion-safe:ease-out motion-safe:zoom-in-75 motion-safe:fade-in" />
      )}
    </div>
  )
}
  ...
  return (
    <div
      className={cn(
        ...
        "text-foreground",
        "motion-safe:animate-in motion-safe:duration-300 motion-safe:ease-out motion-safe:fill-mode-both motion-safe:zoom-in-95 motion-safe:blur-in-sm motion-safe:fade-in",
        className
      )}
    ...
    >
      ...
      >
        <div className="flex items-center justify-between gap-3">
          <Heading className="text-base font-medium">{choice.title}</Heading>
        ...
        <div className="flex flex-col">
          {choice.summary.map((item, index) => (
            <Fragment key={index}>
              ...
              <div
                className="flex flex-col gap-0.5 text-sm motion-safe:animate-in motion-safe:duration-300 motion-safe:ease-out motion-safe:fill-mode-both motion-safe:blur-in-sm motion-safe:fade-in motion-safe:slide-in-from-bottom-1"
...
interface StepContentProps {
  ...
  headingLevel: 2 | 3
  labels: NonNullable<QuestionFlowProgressiveProps["labels"]>
}
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isExiting && [
          "absolute inset-0",
          "motion-safe:animate-out motion-safe:duration-250 motion-safe:ease-[var(--cubic-ease-in-out)] motion-safe:fill-mode-forwards motion-safe:blur-out-sm motion-safe:fade-out",
          exitClass,
        ],
        !isExiting &&
          isTransitioning && [
            "motion-safe:animate-in motion-safe:duration-250 motion-safe:ease-[var(--cubic-ease-in-out)] motion-safe:fill-mode-both motion-safe:blur-in-sm motion-safe:fade-in",
            enterClass,
          ]
      )}
    ...
    >
      <div className="flex flex-col gap-1">
        <Heading id={titleId} className="text-lg leading-tight font-semibold">
          {title}
        </Heading>
        {description && (
...
function StepContent({
  ...
  const stepLabel = labels.step(step, totalSteps)
