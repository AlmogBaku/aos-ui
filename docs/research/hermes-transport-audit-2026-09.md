...
| F30     | Fixed  | Slash-command detection failures are an outage, not a refusal: pinned by slash-commands.test.ts "reports a malformed catalog as an outage instead of a refused command".                                                                                                                               |
| F31     | Fixed  | Each native failure class has its own public code: pinned by adapter.test.ts "publishes each native failure class under its own public error code".                                                                                                                                                    |
| F32     | Fixed  | Browser reconnect backoff is bounded and jittered: pinned by aos-reconnect.test.ts "grows the jitter window exponentially and caps it" and "applies full jitter between zero and the ceiling", with aos-reconnect-stream.test.ts "returns silently when the caller aborts during a reconnect backoff". |
| F33     | Fixed  | A reload during a running segment reconnects instead of stalling: pinned by aos-thread-list.test.ts "keeps a confirmed-running run alive by reconnecting from the start of its segment".                                                                                                               |
| F34     | Fixed  | The browser resumes an uncertain execution the provider still reports: pinned by aos-thread-list.test.ts "resumes an uncertain execution that the provider still reports with a run id".                                                                                                               |
| F35     | Fixed  | A stale cursor restarts rather than skipping events: pinned by aos-reconnect-stream.test.ts "restarts from cursor 0 when a redial delivers a lower sequence id than it sent" and session-coordinator.test.ts "delivers recovered events to two redials sharing one cursor".                            |
| F36     | Fixed  | No sticky status cache outlives the run: pinned by aos-client.test.ts "prefers the provider Session status once no run is observed".                                                                                                                                                                   |
| F37     | Fixed  | A draft Session's first turn is observed through its resolved thread id: pinned by aos-reconnect-stream.test.ts "observes the first turn of a draft Session through the resolved thread id".                                                                                                           |
