...
| Agent creation                  | No           | Native creator workflow | Interview only; safe write blocked | No shared support           |
...
| Published Artifacts             | Demo data    | Native integration      | Native integration                 | Shared presentation tools   |
...
OpenCode owns Agent definitions, Sessions, execution, model credentials, and persistence. AOS supplies a native integration plugin and creator definition. Catalog visibility is currently read-only because the native integration does not expose the required safe mutation. Newly written Agent definitions may require an operator-controlled OpenCode restart before they become ready.

## Hermes

Hermes owns profiles, Sessions, authentication, speech providers, tools, and persistence. AOS connects directly to the native HTTP/WebSocket API. Profile visibility is native. The creator interview is present, but automated profile creation fails closed until Hermes provides an atomic no-overwrite create operation.
